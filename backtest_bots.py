...
# 포트폴리오 초기 자본 (두 봇 동일 기준으로 비교)
BITGET_CAPITAL = 100000.0
...
def get_funding_rates(symbol, silent=False):
    """펀딩비 히스토리 다운로드 (Binance 시도, 차단 시 상수 폴백 사용, 캐시 지원)"""
    ...
    # df가 비어 있으면 map_funding_to_4h에서 상수 폴백(FUNDING_RATE_8H * 0.5)이 사용됨
    return df
...
def _fetch_binanceus_klines(symbol, interval, silent):
    """Binance.US (api.binance.us) 스팟 캔들 다운로드 (GitHub Actions US 서버 접근 가능)"""
...
def get_binance_futures_klines(symbol, interval='4h', silent=False):
    ...
    if df.empty:
        if not silent:
            print(f"    Binance 차단됨 → Binance.US 폴백: {symbol} ({interval})")
        df = _fetch_binanceus_klines(symbol, interval, silent)
    if not df.empty:
        df.to_csv(cache_file, index=False)
    return df
