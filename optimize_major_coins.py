"""
================================================================================
Binance Futures 메이저 코인 롱+숏 독립 파라미터 최적화
================================================================================
대상: BTC, ETH, XRP, SOL, TRX, DOGE, BCH, ADA (8개)

전략 로직 (롱 우선):
  롱 시그널: 시가 > MA_long(4H) AND K_long > D_long (1D, 전일) → 롱 진입
  숏 시그널: 시가 < MA_short(4H) AND K_short < D_short (1D, 전일)
             롱 시그널 없을 때만 숏 진입 (숏은 보조)

최적화 방식: 3단계 독립 최적화
  Stage 1: 롱 파라미터만 최적화 (숏 없이 순수 롱)
  Stage 2: 숏 파라미터 최적화 (롱 파라미터 고정, 결합 백테스트)
  Stage 3: 결합 검증 (롱만 vs 롱+숏 비교)
...
# ==========================================
# Stage 1: 순수 롱 백테스트 (숏 없이)
# ==========================================

def backtest_long_only(data: dict, long_ma: int, long_sk: int,
                       long_sks: int, long_sd: int, long_lev: int) -> dict:
    """
    순수 롱 전략만 백테스트 (Stage 1용)
    - 롱 진입: open > MA_long AND K_long > D_long (전일)
    - 롱 청산: 조건 미충족
    - 포지션 내 비복리, 거래 간 복리
    """
    df_4h = data['df_4h']
    ...
    max_stoch = long_sk + long_sks + long_sd + 10
    if len(df_4h) < long_ma + 50 or len(df_daily) < max_stoch:
        return {'cagr': -999, 'mdd': -100, 'sharpe': -999, 'trades': 0,
                'long_trades': 0, 'short_trades': 0}

    df_sig = prepare_signals(df_4h, df_daily, long_ma, long_sk, long_sks, long_sd)
    ...
    for i in range(1, len(df_bt)):
        ...
        # 롱 시그널: open > MA AND K > D
        long_signal = (opening_price > prev['ma']) and (prev['prev_slow_k'] > prev['prev_slow_d'])
        ...
        # 롱 청산
        elif not long_signal and in_long:
            if entry_price > 0:
    ...
    result = calculate_performance(equity_arr, total_days, trade_count,
                                   long_count=trade_count // 2, short_count=0)
    return result


# ==========================================
# Stage 2/3: 롱+숏 결합 백테스트
# ==========================================

def backtest_combined(data: dict,
                      long_ma: int, long_sk: int, long_sks: int, long_sd: int, long_lev: int,
                      short_ma: int, short_sk: int, short_sks: int, short_sd: int, short_lev: int) -> dict:
    """
    롱+숏 결합 백테스트 (롱 우선, 숏 보조)

    전략:
      if 롱 시그널:
          숏 보유 → 숏 청산 → 롱 진입
      elif 숏 시그널 AND 롱 미보유:
          숏 진입
      else:
          보유 포지션 청산
    """
    ...
    min_ma = max(long_ma, short_ma)

    if len(df_4h) < min_ma + 50 or len(df_daily) < max(max_stoch_long, max_stoch_short):
        return {'cagr': -999, 'mdd': -100, 'sharpe': -999, 'trades': 0,
                'long_trades': 0, 'short_trades': 0}
    ...
    for i in range(1, len(df_bt)):
        ...
        elif in_short and entry_price > 0:
            price_return = curr['close'] / entry_price - 1
...
def optimize_long_phase1(symbol: str, data: dict) -> dict:
    """Stage 1 - Phase 1: 롱만 빠른 스크리닝 (50 trials)"""

    def objective(trial):
        ma = trial.suggest_int('long_ma', *PARAM_RANGES['ma_range'])
        sk = trial.suggest_int('long_sk', *PARAM_RANGES['stoch_k_range'])
        sks = trial.suggest_int('long_sks', *PARAM_RANGES['stoch_smooth_range'])
        sd = trial.suggest_int('long_sd', *PARAM_RANGES['stoch_d_range'])
        lev = trial.suggest_int('long_lev', *PARAM_RANGES['leverage_range'])

        result = backtest_long_only(data, ma, sk, sks, sd, lev)
    ...
    best_result = backtest_long_only(data, best['long_ma'], best['long_sk'],
                                     best['long_sks'], best['long_sd'], best['long_lev'])
...
def optimize_long_phase2(symbol: str, data: dict, phase1_result: dict) -> dict:
    """Stage 1 - Phase 2: 롱만 정밀 파인튜닝 (300 trials)"""
    ...
    def objective(trial):
        ma = trial.suggest_int('long_ma', *ranges_p2['ma_range'])
        sk = trial.suggest_int('long_sk', *ranges_p2['stoch_k_range'])
        sks = trial.suggest_int('long_sks', *ranges_p2['stoch_smooth_range'])
        sd = trial.suggest_int('long_sd', *ranges_p2['stoch_d_range'])
        lev = trial.suggest_int('long_lev', *ranges_p2['leverage_range'])
...
# ==========================================
# Stage 2: 숏 파라미터 최적화 (롱 고정)
# ==========================================

def optimize_short_phase1(symbol: str, data: dict, long_params: dict) -> dict:
    """Stage 2 - Phase 1: 숏 빠른 스크리닝 (롱 고정, 50 trials)"""

    def objective(trial):
        s_ma = trial.suggest_int('short_ma', *PARAM_RANGES['ma_range'])
        s_sk = trial.suggest_int('short_sk', *PARAM_RANGES['stoch_k_range'])
        s_sks = trial.suggest_int('short_sks', *PARAM_RANGES['stoch_smooth_range'])
        s_sd = trial.suggest_int('short_sd', *PARAM_RANGES['stoch_d_range'])
        s_lev = trial.suggest_int('short_lev', *PARAM_RANGES['leverage_range'])

        result = backtest_combined(
            data,
            long_params['long_ma'], long_params['long_sk'],
            long_params['long_sks'], long_params['long_sd'], long_params['long_lev'],
            s_ma, s_sk, s_sks, s_sd, s_lev
        )
    ...
    best_result = backtest_combined(
        ...
        long_params['long_ma'], long_params['long_sk'],
        long_params['long_sks'], long_params['long_sd'], long_params['long_lev'],
        best['short_ma'], best['short_sk'], best['short_sks'], best['short_sd'], best['short_lev']
    )
...
def optimize_short_phase2(symbol: str, data: dict, long_params: dict, phase1_result: dict) -> dict:
    """Stage 2 - Phase 2: 숏 정밀 파인튜닝 (롱 고정, 300 trials)"""
    ...
    def objective(trial):
        s_ma = trial.suggest_int('short_ma', *ranges_p2['ma_range'])
        s_sk = trial.suggest_int('short_sk', *ranges_p2['stoch_k_range'])
        s_sks = trial.suggest_int('short_sks', *ranges_p2['stoch_smooth_range'])
        s_sd = trial.suggest_int('short_sd', *ranges_p2['stoch_d_range'])
        s_lev = trial.suggest_int('short_lev', *ranges_p2['leverage_range'])
...
def main():
    print("=" * 80)
    print("📈 Binance Futures 메이저 코인 롱+숏 독립 파라미터 최적화")
    print("=" * 80)
    ...
    print(f"📌 Stage 1 (롱만): Phase1 {PHASE1_TRIALS} + Phase2 {PHASE2_TRIALS} trials")
    print(f"📌 Stage 2 (숏, 롱 고정): Phase1 {PHASE1_TRIALS} + Phase2 {PHASE2_TRIALS} trials")
    print(f"📌 전략: 롱 우선, 숏 보조 (롱 시그널 시 숏 청산)")
    ...
    for idx, symbol in enumerate(TARGET_COINS, 1):
        ...
        long_p1 = optimize_long_phase1(symbol, data)
        print(f"✅ CAGR:{long_p1['cagr']:.1f}% Lev:{long_p1['long_lev']}x "
              f"MA:{long_p1['long_ma']} SK:{long_p1['long_sk']}/{long_p1['long_sks']}/{long_p1['long_sd']}")

        print(f"    Phase 2 ({PHASE2_TRIALS} trials)...", flush=True)
        long_p2 = optimize_long_phase2(symbol, data, long_p1)
        print(f"    ✅ CAGR:{long_p2['cagr']:.1f}% (개선:{long_p2['improvement']:+.1f}%) "
              f"Lev:{long_p2['long_lev']}x "
              f"MA:{long_p2['long_ma']} SK:{long_p2['long_sk']}/{long_p2['long_sks']}/{long_p2['long_sd']} "
              f"MDD:{long_p2['mdd']:.1f}% Sharpe:{long_p2['sharpe']:.2f}")

        long_only_cagr = long_p2['cagr']
        long_params = {
        ...
        print(f"    Phase 1 ({PHASE1_TRIALS} trials)...", end=" ", flush=True)
        short_p1 = optimize_short_phase1(symbol, data, long_params)
        print(f"✅ Combined CAGR:{short_p1['cagr']:.1f}% "
              f"MA:{short_p1['short_ma']} SK:{short_p1['short_sk']}/{short_p1['short_sks']}/{short_p1['short_sd']} "
              f"Lev:{short_p1['short_lev']}x "
              f"롱:{short_p1['long_trades']}회 숏:{short_p1['short_trades']}회")

        print(f"    Phase 2 ({PHASE2_TRIALS} trials)...", flush=True)
        short_p2 = optimize_short_phase2(symbol, data, long_params, short_p1)
        print(f"    ✅ Combined CAGR:{short_p2['cagr']:.1f}% (개선:{short_p2['improvement']:+.1f}%) "
              f"MA:{short_p2['short_ma']} SK:{short_p2['short_sk']}/{short_p2['short_sks']}/{short_p2['short_sd']} "
              f"Lev:{short_p2['short_lev']}x MDD:{short_p2['mdd']:.1f}% Sharpe:{short_p2['sharpe']:.2f} "
              f"롱:{short_p2['long_trades']}회 숏:{short_p2['short_trades']}회")
        ...
        combined_result = backtest_combined(
            ...
            long_params['long_ma'], long_params['long_sk'],
            long_params['long_sks'], long_params['long_sd'], long_params['long_lev'],
            short_p2['short_ma'], short_p2['short_sk'],
            short_p2['short_sks'], short_p2['short_sd'], short_p2['short_lev']
        )

        combined_cagr = combined_result['cagr']
        diff = combined_cagr - long_only_cagr
        better = "✅ 숏 추가 효과" if diff > 0 else "⚠️ 숏 추가 시 성과 하락"

        print(f"    롱만 CAGR:   {long_only_cagr:.1f}%")
        ...
        result_row = {
            ...
            'Days': data['days'],
            'Long_MA': long_params['long_ma'],
            'Long_SK': long_params['long_sk'],
            'Long_SKs': long_params['long_sks'],
            'Long_SD': long_params['long_sd'],
            'Long_Lev': long_params['long_lev'],
    ...
    csv_path = os.path.join(SAVE_DIR, f"binance_major_coins_optimized_{ts}.csv")
    ...
    print(f"📊 최종 결과 요약")
    print(f"{'=' * 130}")
    print(f"{'Symbol':<12} {'L_Lev':>5} {'L_MA':>5} {'L_SK':>5} {'L_SKs':>5} {'L_SD':>5} "
          f"{'S_Lev':>5} {'S_MA':>5} {'S_SK':>5} {'S_SKs':>5} {'S_SD':>5} "
          f"{'L_CAGR%':>8} {'C_CAGR%':>8} {'C_MDD%':>7} {'Sharpe':>7} "
          f"{'총거래':>6} {'롱':>4} {'숏':>4}")
    print(f"{'─' * 130}")
    ...
    for _, row in df_results.iterrows():
        print(f"{row['Symbol']:<12} "
              ...
              f"{row['Long_CAGR']:>7.1f}% {row['Combined_CAGR']:>7.1f}% "
              f"{row['Combined_MDD']:>6.1f}% {row['Combined_Sharpe']:>7.2f} "
              f"{row['Combined_Trades']:>6} {row['Long_Trades']:>4} {row['Short_Trades']:>4}")

    # ── 봇 적용용 COMBINED_TRADING_CONFIGS 출력 ──
    print(f"\n{'=' * 80}")
    print("📋 MAJOR_TRADING_CONFIGS (봇 적용용)")
    print(f"{'=' * 80}")
    ...
    for _, row in df_results.iterrows():
        print(f"    {{'symbol': '{row['Symbol']}', "
              ...
              f"'long_lev': {row['Long_Lev']}, 'short_lev': {row['Short_Lev']}}},")
    print("]")
    ...
    print(f"📊 통계 요약")
    print(f"{'=' * 80}")
    print(f"평균 Long-only CAGR: {df_results['Long_CAGR'].mean():.1f}%")
    ...
    improved = len(df_results[df_results['Combined_CAGR'] > df_results['Long_CAGR']])
    print(f"\n숏 추가 효과: {improved}/{len(df_results)}개 코인 개선")
